 ...
 */
const JWKS_URL = 'https://uptodolist.auth0.com/.well-known/jwks.json'
 ...
 */
async function getSigningKey(kid) : Promise<SigningKey> {
    if (!SIGNING_KEYS) {
        SIGNING_KEYS = await getSigningKeys()
    }
 ...
 */
async function getSigningKeys() : Promise<Array<SigningKey>> {
    return getJwks().then(
        (jwks) => {
            if (!jwks || !jwks.length) {
            ...
            const signingKeys = jwks
                .filter(jwk => jwk.use === 'sig'                            // JWK property `use` determines the JWK is for signing
                    ...
                    return {
                        kid: jwk.kid,
                        nbf: undefined,
                        publicKey: certToPEM(jwk.x5c[0])    // Using the x5c property build a certificate to be used to verify the JWT signature
                    }
                })
 ...
 */
async function getJwks() : Promise<Array<Jwk>> {
